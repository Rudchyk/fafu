...
<section role="main" class="section white_block clearfix">
	<div class="content left">
		...
		<div class="b-posts block coach-team">
			<h2 class="title cat-title"><?php _e('Тренерский штаб сборной', 'kubrick'); ?> <a href="/category/trenerskij-shtab-sbornoj/" class="icon more-icon"></a></h2>
			<table class="roster-table">
				...
				<?php
				$args = array( 'numberposts' => 1, 'posts_per_page' => 50, 'category' => "52" );
				$lastposts = get_posts( $args );
				foreach($lastposts as $post) : setup_postdata($post); ?>
				<tr class="roster-row">
					<td class="roster-cell roster-p-name"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></td>
					<td class="roster-cell roster-p-status"><? echo get_post_meta($post->ID, 'status-national-team', true); ?></td>
		...
		<div class="b-posts block">
			...
			<table class="roster-table">
				...
				<?php
				$args = array( 'numberposts' => 1, 'posts_per_page' => 50, 'category' => "58" );
				$lastposts = get_posts( $args );
				foreach($lastposts as $post) : setup_postdata($post); ?>
				<tr class="roster-row">
					<td class="roster-cell roster-p-number"><? echo get_post_meta($post->ID, 'number', true); ?></td>
