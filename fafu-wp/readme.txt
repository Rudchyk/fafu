middle-menu.php - Баннеры на главной
main-block.php - Слайдер и блок с новостями
future-games.php - Будущие игры
last-games.php - Прошедшие игры
team-list.php - Cписок команд в хедере 
partners-widget.php - Виджет "Партнерство"
friends.php - блок "Наши друзья"
main-video.php - Блок "Свежее видео"
main-players.php - Блок "Члены сборной Украины"
nav-widget.php - Виджет рубрики
category-5.php - Категория "Ростер"
category-6.php - Категория "Тренеры"
category-7.php - Категория "Видео"
main-player.php - Шаблон игрока

font-family: 'Open Sans Condensed', sans-serif;

Light 300
Light 300 Italic
Bold 700

9px	- .75em
10px - .833333333333333em
11px - .916666666666667em
12px - 1em
13px - 1.08333333333333em
14px - 1.16666666666667em
15px - 1.25em
16px - 1.33333333333333em
17px - 1.41666666666667em
18px - 1.5em
19px - 1.58333333333333em
20px - 1.66666666666667em
21px - 1.75em
22px - 1.83333333333333em
23px - 1.91666666666667em
24p - 2em
25px - 2.08333333333333em
26px - 2.16666666666667em
27px - 2.25em
28px - 2.33333333333333em
29px - 2.41666666666667em
30px - 2.5em

Произвольные поля:

Игра:
home-team - Домашняя команда
	wolves - Волки
	mustangs - Мустанги
	dolphins - Дельфины
	lions - Львы
	loggers - Лесорубы
	monarchs - Монархи
	atlantes - Атланты
	bandits - Бандиты
	knights - Витязи
	pirates - Пираты
	rockets - Ракеты
	scythians - Скифы
	slavs - Славяне
	tanks - Танки
	tauris - Тавры
	zubrs - Минские зубры
	vikings - Викинги
	bisons - Бизоны
	gepards - Гепарды
	sharks - Акулы
	gladiators - Гладиаторы
	bears - Мелитопольские Медведи
	hammers -  "Молотобойцы" (Донецк)

home-team-name - Имя домашней команды
home-team-city - Город домашней команды

visited-team - Гостевая команда
	wolves - Волки
	mustangs - Мустанги
	dolphins - Дельфины
	lions - Львы
	loggers - Лесорубы
	monarchs - Монархи
	atlantes - Атланты
	bandits - Бандиты
	knights - Витязи
	pirates - Пираты
	rockets - Ракеты
	scythians - Скифы
	slavs - Славяне
	tanks - Танки
	tauris - Тавры
	zubrs - Минские зубры
	vikings - Викинги
	bisons - Бизоны
	gepards - Гепарды
	sharks - Акулы
	gladiators - Гладиаторы
	bears - Мелитопольские Медведи

visited-team-name - Имя гостевой команды
visited-team-city - Город гостевой команды

championship - Чемпионат
	higher - Высшая лига
	first - Первая лига
	cup - Кубок Украины
	flag - Флаг-футбол
	arena - Донбасс-Аренабол зимний чемпионат
	friendly - Товарищеская игра

game-day - День матча
game-score - Счет матча
home-team-first-quarter - Очки I четверти домашней команды
home-team-second-quarter - Очки II четверти домашней команды
home-team-third-quarter - Очки III четверти домашней команды
home-team-four-quarter - Очки IV четверти домашней коанды
visited-team-first-quarter - Очки I четверти гостевой команды
visited-team-second-quarter - Очки II четверти гостевой команды
visited-team-third-quarter - Очки III четверти гостевой команды
visited-team-four-quarter - Очки IV четверти гостевой команды

Персона:
photo - Фото 122*159
person - Игровая фотография 958*250
pos - Позиция (Условное обозначение)
position - Позиция (Полное название)
number - Игровой номер
ht - Рост
wt - Вес
age - Возраст (Дата рождения)
exp - Опыт (Датя начала занятий АФ)
team - Команда
	wolves - Волки
	mustangs - Мустанги
	dolphins - Дельфины
	lions - Львы
	loggers - Лесорубы
	monarchs - Монархи
	atlantes - Атланты
	bandits - Бандиты
	knights - Витязи
	pirates - Пираты
	rockets - Ракеты
	scythians - Скифы
	slavs - Славяне
	tanks - Танки
	tauris - Тавры
	zubrs - Минские зубры
	vikings - Викинги
	bisons - Бизоны
	gepards - Гепарды
	sharks - Акулы
	gladiators - Гладиаторы
	bears - Мелитопольские Медведи
team-name - Имя команды
city - Город
status - Статус
status-national-team - Статус тренера в сборной
sort - Сортировка (Пр.: 01)

Видео:
video-img - Картинка на видео 314*146
Видео должно быть размеров width="633" height="356"

Главные новости:
slide - Картинка на слайдер 675*318

Новость:
Превью не меньше 190px.
h-142

Команда:
team - Команда
	wolves - Волки
	mustangs - Мустанги
	dolphins - Дельфины
	lions - Львы
	loggers - Лесорубы
	monarchs - Монархи
	atlantes - Атланты
	bandits - Бандиты
	knights - Витязи
	pirates - Пираты
	rockets - Ракеты
	scythians - Скифы
	slavs - Славяне
	tanks - Танки
	tauris - Тавры
	zubrs - Минские зубры
	vikings - Викинги
	bisons - Бизоны
	gepards - Гепарды
	sharks - Акулы
	gladiators - Гладиаторы
	bears - Мелитопольские Медведи
team-name-en - Англ. название
city - Город
date - Дата основания
children - Детская команда
website - Сайт
team-photo - Фото команды 980*479 (450*220)
progress - Достижения
contacts - Контакты
team-art - Фотография игры команды 958*250

Классы:
pdf - добавляет иконку PDF файла 